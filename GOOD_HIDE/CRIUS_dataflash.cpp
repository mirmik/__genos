...
#include <genos/board/CRIUS_dataflash.h>
...
{ 
uint8 status;
genos::spi_transaction* spi_trans = spi_driver.create_transaction(&CRIUS_dataflash);
spi_trans->gs.reserve(2);
spi_trans->gs << 0x57 << 0x00;
spi_driver.add_transaction(spi_trans);

while (!spi_trans->status.sts(genos::IS_FINISHED));
 
 status = spi_trans->gs[1];
 spi_trans->destruct();
...
uint8 df_GetChar2(uint8 BufferNo, uint16 Addr )
{
uint8 data;
genos::spi_transaction* spi_trans = spi_driver.create_transaction(&CRIUS_dataflash);
spi_trans->gs.reserve(6);
spi_trans->gs << Buf1Read << 0x00 << (unsigned char)(Addr>>8) << (unsigned char)(Addr) << 0x00 << 0x00;
spi_trans->status.set(genos::IS_RETURNED);
spi_driver.add_transaction(spi_trans);
while (!spi_trans->status.sts(genos::IS_FINISHED));
 data = spi_trans->gs[5];
 spi_trans->destruct();
...
void df_WriteByte2( uint8 BufferNo, uint16 Addr, uint8 Data )
{
genos::spi_transaction* spi_trans = spi_driver.create_transaction(&CRIUS_dataflash);

spi_trans->gs.reserve(5);
spi_trans->gs << Buf1Write << 0x00 << (unsigned char)(Addr>>8) << (unsigned char)(Addr) << Data;
spi_driver.add_transaction(spi_trans);
spi_trans->destruct();
}
